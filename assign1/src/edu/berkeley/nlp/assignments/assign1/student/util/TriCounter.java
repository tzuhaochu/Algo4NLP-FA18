...
public final class TriCounter implements Counter {
    static public Indexer indexer;
    public int[] content;

    public TriCounter(int offset, double loadFactor) {
        int shift = (int)(offset / 8.0 * 11.0);
        int length = (int)((1L << shift) / loadFactor);
        content = new int[length];
        indexer = new Indexer(length, offset * 3);
    }
